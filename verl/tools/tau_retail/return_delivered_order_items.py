...
class ReturnDeliveredOrderItems(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if order["status"] != "delivered":
            return ToolResponse(text="Error: non-delivered order cannot be returned"), 0.0, {}
        ...
        if (
            ...
            return ToolResponse(text="Error: payment method should be either the original payment method or a gift card"), 0.0, {}

        # Check if the items to be returned exist (there could be duplicate items in either list)
        all_item_ids = [item["item_id"] for item in order["items"]]
        for item_id in item_ids:
            if item_ids.count(item_id) > all_item_ids.count(item_id):
                return ToolResponse(text="Error: some item not found"), 0.0, {}
