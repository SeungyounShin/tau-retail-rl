...
class GetUserDetails(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if "users" not in data:
            return ToolResponse(text="Error: data.users is missing"), 0.0, {}
        ...
        if raw_user_id is None:
            return ToolResponse(text="Error: parameter 'user_id' is required"), 0.0, {}

        if isinstance(raw_user_id, dict):
            raw_user_id = raw_user_id.get("id") or raw_user_id.get("user_id")
            if raw_user_id is None:
                return ToolResponse(text="Error: parameter 'user_id' must be a string or an object with 'id'/'user_id'"), 0.0, {}
        ...
        if isinstance(users, list):
            for u in users:
                if isinstance(u, dict) and str(u.get("id") or u.get("user_id")) == user_id:
                    return ToolResponse(text=json.dumps(u)), 0.0, {}
            return ToolResponse(text="Error: user not found"), 0.0, {}

        return ToolResponse(text="Error: data.users must be a dict or a list"), 0.0, {}
